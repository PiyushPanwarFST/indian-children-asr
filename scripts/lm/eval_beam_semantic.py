"""
Semantic Branch: Greedy vs Beam+LM CTC Decoding — Full Test Set
================================================================
Runs ONLY our semantic encoder + CTC head (char vocab).
Compares greedy vs beam+LM decode on the full test set.

Usage:
    python scripts/lm/eval_beam_semantic.py
    python scripts/lm/eval_beam_semantic.py --alpha 1.0 --beta 2.0
"""
...
parser.add_argument("--checkpoint", type=str, default="checkpoints/semantic_ctc_unfrozen/best_dev.pt")
...
print("=" * 70)
print("  Semantic Branch: Greedy vs Beam+LM")
print("=" * 70)
...
# Semantic checkpoints use "ctc_head_char_state_dict"
if "ctc_head_char_state_dict" in ckpt:
    ctc_head.load_state_dict(ckpt["ctc_head_char_state_dict"])
elif "ctc_head_state_dict" in ckpt:
    ctc_head.load_state_dict(ckpt["ctc_head_state_dict"])
else:
    raise KeyError(f"No CTC head found in checkpoint. Keys: {list(ckpt.keys())}")

epoch = ckpt.get("epoch", "?")
dev_wer = ckpt.get("dev_wer", 0)
print(f"  Loaded epoch {epoch}, dev WER: {dev_wer:.2f}%")
...
# ── Results ──────────────────────────────────────────────────────────────
print(f"\n{'='*70}")
print(f"  RESULTS — SEMANTIC BRANCH — {args.split.upper()} SET")
print(f"{'='*70}")
